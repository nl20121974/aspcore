...
class htmlhandlerclass
    ...
    public function btn(p_href)
        set btn = (new htmlanchorclass).href(p_href).css(m_styles("btn"))
    end function
    ...
    public function checklist(p_name)
        ...
        items.MoveFirst
        set checklist = (new htmllistclass).name(p_name).id(p_name).Items(items).key("Value").text("text").css(m_styles("checklist"))
    end function
